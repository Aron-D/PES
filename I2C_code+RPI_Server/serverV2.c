...
void handleMsg(char* key, int value, int s, device* sensors, device* actuators) {
	char* msg = "OK\n";
	if (strcmp(key, "Temperatuur") == 0) {
...
//loop socket connecties
	while (TRUE){
		...
		char hello[100];
		sprintf(hello , "%s : %.2lf, %s : %.2lf, %s : %.2lf", sensors[1].name, sensors[1].value, sensors[2].name, sensors[2].value, sensors[3].name, sensors[3].value);
        ...
        i2cBusAansturing(sensors, actuators); //gebruikt de waardes van de struct
