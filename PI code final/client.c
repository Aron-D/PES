...
void handleMsg(char* key, char* value, int s, device* sensors, device* actuators) {
	...
	} else if (strcmp(key, "bedradeDrukSensor") == 0) {
		sensors[9].value = atof(value); //bedradeDrukSensor
	} else if (strcmp(key, "grondgevoeligeSensor") == 0) {
		sensors[10].value = atof(value); //grondgevoeligeSensor
	} else {
...
int main(int argc, char const* argv[])
{
	...
	char* server_ips[] = { "10.42.0.1" , "10.42.0.25", "10.42.0.123", "10.42.0.108", "10.42.0.102", "10.42.0.27", "10.42.0.83", "10.42.0.148" }; // voeg adressen toe per WEMOS
	int num_ips = sizeof(server_ips) / sizeof(server_ips[0]);
	//Bestandsnaam declaratie
	FILE* sensorFile;
	FILE* actuatorFile;
	char* sensorFileName = "sensorFile";
	char* actuatorFileName = "actuatorFile";
	//time_t t = time(NULL);
	//controle of de deur al dicht mag na bepaalde tijd
	int isDoorOpen = 0;
	int doorOpenSecond = 0;
	int doorCounter = 0;
	//controle of de bewoner te lang in bed ligt
	int inBed = 0;
	int timeInBedSecond = 0;
	char bedTime[20];
	int bedMin = 0;
	int bedSec = 0;
	//controle of de rfid pas klopt.
	char rfidWaarde[8];
	char RFIDAttempt[500];	
	char lastInvalidRFIDAttempt[500];
	char bedNat[10];
				
	/*
	if(sensorFile == NULL || actuatorFile == NULL){
		printf("Could not read device sensors/n");
		return 1;
	}
	*/
		...
		switch(socketCounter % num_ips) {
			...
			case 4: //servomotor
				...
				if (isDoorOpen == 1) {
					...
					if(doorOpenSecond > 54){
						if(tm.tm_sec < 60){
							if (doorOpenSecond + 4 < tm.tm_sec + 60) {
								/*doorCounter++;
								doorOpenSecond = tm.tm_sec;
								if (doorCounter > 4) {
									doorCounter = 0;
					...
					} else if (doorOpenSecond + 4 < tm.tm_sec) {
						/*doorCounter++;
						doorOpenSecond = tm.tm_sec;
						if (doorCounter > 4) {
							doorCounter = 0;
				...
				if (strcmp(rfid, "D935D814") == 0) {
					...
					printf("Valid RFID attempt\n\r");
				} else if (strcmp(rfid, "") == 0) {
					//ignorar
				} else {
					sensors[5].value = 0;
					sprintf(RFIDAttempt, "[%s] Invalid login attempt on %s \n", rfid, dateBuffer);
					printf("Invalid RFID attempt\n\r");
					strcpy(lastInvalidRFIDAttempt, dateBuffer);
				}
				if (strcmp(rfid, "") != 0) {
					printf("Logging: %s\r\n", RFIDAttempt);
			...
			case 5: //lichtkrant
				...
				if(tm2.tm_min < 10){
					sprintf(lichtkrantTijd, "%d:0%d", tm2.tm_hour, tm2.tm_min);					
				}
				else{
				...
				break;
			case 6: //bewegings sensor
				sprintf(hello, "Wat is je waarde");
				break;
			case 7://speciaalbeheerder
			//start timer als de druksensor druk meet
			//verander voor echte wereld tijd naar aantal uren & minuten ipv minuten & seconden 
				if (sensors[9].value == 0 && inBed ==0) {
					inBed = 1;
					time_t t = time(NULL);
					struct tm tm3 = *localtime(&t);
					timeInBedSecond = tm3.tm_sec; 
					sprintf(bedTime, "%d:%d", bedMin, bedSec);
				}
			//Vergelijk of er al een seconde voorbij is
				if (sensors[9].value == 0) {
					time_t t = time(NULL);
					struct tm tm3 = *localtime(&t);
					printf("Comparing timeInBed second %d to new second %d\n\r", timeInBedSecond, tm3.tm_sec);
					if (timeInBedSecond < tm3.tm_sec) {
						bedSec++;
						timeInBedSecond = tm3.tm_sec;	
					}					
				}
			...
			//reset timer als de druksensor geen druk meer meet
				if (sensors[9].value != 0){
					inBed = 0;
					bedSec = 0;
					bedMin = 0;
			...
			//is het bed nat?
				if (sensors[7].value < 1500) {
					strcpy(bedNat, "nat");
				} else {
					strcpy(bedNat, "droog");
				}
				sprintf(bedTime, "%d:%d", bedMin, bedSec);
				sprintf(hello, "Temp : %.1lf , Hum : %.1lf , CO2 : %.0lf , Laatste invalid RFID poging : %s , tijd in bed : %s , bed nat: %s\n\r", sensors[2].value, sensors[3].value, sensors[6].value, lastInvalidRFIDAttempt, bedTime, bedNat);
				memset(bedNat, 0, 10);
				break;
		}
		printf("Sending: %s\n", hello);
