...
//todo: add socket to parameters
void ServerSocket::respond(string flag, FieldBus* bus) {
	string response;	
	if (flag == "hoi") {
		//ack
	} else if (flag == "REQ_TEMP") {
		response = bus->requestVal("TEMP");
	} else if (flag == "REQ_HUM") {
		response = bus->requestVal("HUM");
	} else if (flag == "REQ_CO2") {
		response = bus->requestVal("CO2");
	} else if (flag == "REQ_MSTR"){
		response = bus->requestVal("MSTR");
	} else if (flag == "REQ_PRS") {
		response = bus->requestVal("PRS");
	} else {
...
void ServerSocket::communicate(FieldBus* bus) {
