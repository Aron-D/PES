	...
	while(TRUE){
		...
		switch(socketCounter % num_ips) {
			...
			//reset timer als de druksensor geen druk meer meet
				...
				sprintf(bedTime, "%d:%d", bedMin, bedSec);
				sprintf(hello, "Temp : %.1lf , Hum : %.1lf , CO2 : %.0lf , Laatste invalid RFID poging : %s , tijd in bed : %s\n\r", sensors[2].value, sensors[3].value, sensors[6].value, lastInvalidRFIDAttempt, bedTime);
				break;
		}
