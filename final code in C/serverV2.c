...
int main(int argc, char* argv[]) {
	...
	while (TRUE) {
		...
		if (readOk == 1){
			printf("Unable to read from sensor file (uitlezen niet meow)\n");
		}
		
		readOk = readFromFile(actuatorFile, actuators, actuatorFileName);
		if (readOk == 1){
			printf("Unable to read from actuator file (uitlezen niet meow)\n");
		}
	
		char hello[500];
		sprintf(hello , "%s : %.2lf, %s : %.2lf, %s : %.2lf, %s : %.2lf, %s : %.2lf", sensors[1].name, sensors[1].value, sensors[2].name, sensors[2].value, sensors[3].name, sensors[3].value, sensors[6].name, sensors[6].value, sensors[5].name, sensors[5].value);
